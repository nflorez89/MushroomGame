...
MessageDialog::MessageDialog()
{
	...
	width = messageDialogNS::BUTTON_WIDTH + messageDialogNS::BUTTON_WIDTH/4;//messageDialogNS::WIDTH;
...
void MessageDialog::prepareVerts()
{
	...
	if(buttonType != messageDialogNS::UNIT && buttonType != messageDialogNS::NO_SPAWN_SPACE)
	{
		// button2 top left
		vtx[0].x = x + width/2.0f - messageDialogNS::BUTTON_WIDTH/2.0f;
		vtx[0].y = y + height - messageDialogNS::BORDER - (2*messageDialogNS::MARGIN) - (2*messageDialogNS::BUTTON_HEIGHT);
		vtx[0].z = 0.0f;
		vtx[0].rhw = 1.0f;
		vtx[0].color = buttonColor;
		// button2 top right
	...
	if (buttonType == 0/*menuType == messageDialogNS::MUSHROOM*/)
	{
		// button3 top left
		vtx[0].x = x + width/2.0f - messageDialogNS::BUTTON_WIDTH/2.0f;
		vtx[0].y = y + height - messageDialogNS::BORDER - (3*messageDialogNS::MARGIN) - (3*messageDialogNS::BUTTON_HEIGHT);
		vtx[0].z = 0.0f;
		vtx[0].rhw = 1.0f;
		vtx[0].color = buttonColor;
		// button3 top right
		...
		vtx[3].rhw = 1.0f;
		vtx[3].color = buttonColor;
		graphics->createVertexBuffer(vtx, sizeof vtx, button3Verts);

		// set button3Rect
		button3Rect.left   = (long)vtx[0].x;
		button3Rect.right  = (long)vtx[1].x;
		button3Rect.top    = (long)vtx[0].y;
		button3Rect.bottom = (long)vtx[2].y;
	}
}
...
const void MessageDialog::draw()
{
	...
	graphics->drawQuad(button3Verts);		// draw button3
	...
	if (buttonType == 0/*menuType == messageDialogNS::MUSHROOM*/)
	{
		dxFont.print(messageDialogNS::BUTTON3_TEXT,button3Rect,
                 DT_SINGLELINE|DT_CENTER|DT_VCENTER);
	}
...
void MessageDialog::update()
{
    ...
    if (graphics->getFullscreen() == false) // if windowed
    {
        ...
        screenRatioX = (float)FULL_GAME_WIDTH / clientRect.right;
        screenRatioY = (float)FULL_GAME_HEIGHT / clientRect.bottom;
    }
	
	if (input->getMouseLButtonRelease())       // if mouse left button
    {
		...
		if (buttonType != messageDialogNS::NO_SPAWN_SPACE && input->getMouseX()*screenRatioX >= button2Rect.left &&
		...
		// if mouse clicked inside button3
        if (buttonType == 0/*menuType == messageDialogNS::MUSHROOM*/ &&
			input->getMouseX()*screenRatioX >= button3Rect.left &&
            input->getMouseX()*screenRatioX <= button3Rect.right &&
            input->getMouseY()*screenRatioY >= button3Rect.top &&
            input->getMouseY()*screenRatioY <= button3Rect.bottom)
        {
            visible = false;            // hide message dialog
            buttonClicked = 3;          // button2 was clicked
        }
    }
}
...
void MessageDialog::print(const std::string &str)         
{
	...
	textRect.right  = (long)(x + (messageDialogNS::BUTTON_WIDTH*1.25f)/*messageDialogNS::WIDTH*/ - messageDialogNS::MARGIN);
    textRect.top    = (long)(y + messageDialogNS::MARGIN);
	textRect.bottom = (long)(y + messageDialogNS::BUTTON_HEIGHT - messageDialogNS::MARGIN);
    ...
    height = (textRect.bottom - (int)y + messageDialogNS::BORDER + messageDialogNS::MARGIN) + (textRect.bottom - (int)y + messageDialogNS::BORDER + messageDialogNS::MARGIN)/4;
